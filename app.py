...
def logout():
    ...
    if session[CURR_USER_KEY]:
        do_logout()
    flash('You have logged out, hope to see you soon!')
...
def profile():
        ...
        user_id = session[CURR_USER_KEY]  # Get the user ID from the session
        ...
        # Prepopulate the form with the current user data
...
def show_message(message_id):
    ...
    is_liked = g.user and Likes.query.filter_by(user_id=g.user.id, message_id=msg.id).first() is not None
...
def add_like(message_id):
    ...
    if g.user:
        try:
            ...
            db.session.rollback()
            flash("Failed to like the message due to database integrity error.", "danger")
        except Exception as e:
            db.session.rollback()
            flash(f"Failed to like the message: {str(e)}", "danger")
    else:
...
def homepage():
    ...
    if g.user:
        ...
        followed_users_subquery = (db.session
        ...
        messages = (db.session
                    .query(Message)
                    .filter(Message.user_id.in_(followed_users_subquery))
                    .order_by(Message.timestamp.desc())
                    .limit(100)
                    .all())
